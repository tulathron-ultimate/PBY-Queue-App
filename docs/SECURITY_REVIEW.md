...
- **SEC-10:** If the owner agrees, answer a duplicate phone with "You're already in line; check
  your texts" and re-send the join text (subject to the Twilio cap) instead of returning the
  link, or return the link only to the device that created the party.
