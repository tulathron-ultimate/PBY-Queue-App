...
| `TRUST_PROXY`                                                                              | `true`                                                 | Trust `X-Forwarded-*` from Cloudflare Tunnel or another proxy.                                                                                                                     |
...
3. **Cloudflare Tunnel** (no open ports on your router):
   ...
   - Set `PUBLIC_URL=https://q.yourdomain.com`. `TRUST_PROXY=true` (the default) lets the app
     see HTTPS, so the session cookie is marked Secure.
